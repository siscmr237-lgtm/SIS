...
export function SchoolSettings() {
  ...
  const handleBasicInfoSave = async () => {
    ...
    try {
      ...
      const next: any = { ...settings, name: formData.name, logo: formData.logo, motto: formData.motto, abbreviation: resolvedAbbreviation };
      if (termFieldsDirty) {
  ...
  const handleAbbreviationAutoToggle = async (checked: boolean) => {
    ...
    const liveAbbreviation = computeSchoolAbbreviation(isEditingBasic ? formData.name : settings.name);

    if (checked) {
      setSettings(prev => ({ ...prev, abbreviationAutoGenerated: true, abbreviation: liveAbbreviation } as any));
      try {
        await api.put('/settings', { abbreviationAutoGenerated: true });
        syncLocalStorageSchool({ abbreviationAutoGenerated: true, abbreviation: liveAbbreviation });
        toast.success('Auto-generate enabled — abbreviation now follows the school name.');
      } catch {
        setSettings(prev => ({ ...prev, abbreviationAutoGenerated: false } as any));
        toast.error('Failed to enable auto-generate');
      }
      return;
    }

    setSettings(prev => ({ ...prev, abbreviationAutoGenerated: false, abbreviation: liveAbbreviation } as any));
    setFormData(prev => ({ ...prev, abbreviation: liveAbbreviation }));
    try {
      await api.put('/settings', { abbreviationAutoGenerated: false, abbreviation: liveAbbreviation });
      syncLocalStorageSchool({ abbreviationAutoGenerated: false, abbreviation: liveAbbreviation });
      toast.success('Auto-generate disabled — abbreviation is now set manually.');
    } catch {
      setSettings(prev => ({ ...prev, abbreviationAutoGenerated: true } as any));
      toast.error('Failed to disable auto-generate');
    }
  };
