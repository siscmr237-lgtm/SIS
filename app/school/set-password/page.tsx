...
//
// Deliberately OUTSIDE app/school/(app): this page is opened from an email
// invite, so there is no session yet and the school auth gate would bounce it
// to /school/login before the token could ever be read. Same reasoning, and the
// same shape, as app/teacher/set-password/page.tsx.
...
type VerifyState =
  | { name: "checking" }
  | { name: "valid"; greeting: string | null; schoolName: string | null }
...
function SetPasswordForm() {
  ...
  useEffect(() => {
    ...
    api
      .post("/auth/admin/invite/verify", { token })
      .then((res: any) => {
        if (!alive) return;
        setVerify({
          name: "valid",
          greeting: res?.name || null,
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res: any = await api.post("/auth/admin/set-password", { token, password });
      if (res?.token) {
        // The endpoint hands back a real session, so there is no reason to make
        // someone who just proved ownership of the invite log in again. The
  ...
  return (
    <>
      <div className="text-center md:text-left" style={{ marginBottom: "1.75rem" }}>
        ...
        <p className="text-sm" style={{ color: "#6B7280", marginTop: "0.375rem" }}>
          {verify.schoolName
            ? `Choose a password to finish setting up your administrator account for ${verify.schoolName}.`
            : "Choose a password to finish setting up your administrator account."}
        </p>
      </div>
